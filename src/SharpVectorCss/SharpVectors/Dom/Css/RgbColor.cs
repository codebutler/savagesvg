	...
	public class RgbColor : IRgbColor
	{
		...
		public RgbColor(string str)
		{
			...
			else
			{
				...
				try
				{
					gdiColor = ColorTranslator.FromHtml(str);
				}
				catch
				{
					gdiColor = Color.Black;
				}
		...
		public Color GdiColor
		{
			get
			{
				int red = Convert.ToInt32(Red.GetFloatValue(CssPrimitiveType.Number));
				int green = Convert.ToInt32(Green.GetFloatValue(CssPrimitiveType.Number));
				int blue = Convert.ToInt32(Blue.GetFloatValue(CssPrimitiveType.Number));
