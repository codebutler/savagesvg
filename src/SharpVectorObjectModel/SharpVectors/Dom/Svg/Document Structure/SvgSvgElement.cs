...
namespace SharpVectors.Dom.Svg
{
  ...
  /// </summary>
  public class SvgSvgElement : SvgTransformableElement, ISvgSvgElement, IContainerElement
  {
    #region Constructors
      ...
      get 
      {
        if (cachedViewBoxTransform == null) 
        {
          ...
          if (this != doc.RootElement) 
          {
            ...
            matrix = matrix.Translate((float)X.AnimVal.Value, (float)Y.AnimVal.Value);
          }
          ...
          {
            matrix = matrix.ScaleNonUniform((float)x_ratio, (float)y_ratio);
          }
          else 
          {
            ...
            matrix = matrix.Translate((float)x_trans, (float)y_trans);
            matrix = matrix.Scale((float)x_ratio);
          }
    ...
    private ArrayList redrawTimers = new ArrayList();
    ...
    public void UnsuspendRedraw(int suspendHandleId)
    {
      ...
      if (OwnerDocument.Window.Renderer.InvalidRect != RectangleF.Empty) 
    ...
    {
      OwnerDocument.Window.Renderer.InvalidRect = RectangleF.Empty;
      OwnerDocument.Window.Renderer.Render((ISvgDocument)OwnerDocument);
    }
